...
def write_s3(df, bucket, name):
    ...
    LOG.info(f"INSIDE WRITE_S3!!!")
    csv_buffer = StringIO()
    df.to_csv(csv_buffer)
    LOG.info(f"AFTER CSV_BUFFER!!!")
    s3_resource = boto3.resource('s3')
    filename = f"{name}_sentiment.csv"
    LOG.info(f"BEFORE STORING!!!")
    ...
    # filename = f"{name}_sentiment.csv"
    # LOG.info(f"BEFORE STORING!!!")
    # res = s3_client.put_object(Body=csv_buffer.getvalue(), Bucket=bucket, Key=filename)
    # LOG.info(f"result of write to bucket: {bucket} with:\n {res}")
